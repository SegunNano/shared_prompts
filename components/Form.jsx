...
const Form = ({ type, post, setPost, submitting, handleSubmit }) => {
    return (
        <section className="w-full max-w-full flex-start flex-col">
            ...
            <form onSubmit={handleSubmit} className="mt-10 w-full max-w-2xl flex flex-col gap-7 glassmorphism shadow-[inset_10px_-50px_94px_0_rgb(199, 199, 199, 0.2)] ">
                <label htmlFor="">
                ...
                <label htmlFor="">
                    <span className="font-satoshi font-semibold text-base text-gray-700">Tag {'  '}
                        <span className="font-normal">(#idea, #product)</span></span>
                ...
                <div className="flex-end mx-3 mb-5 gap-4">
                    ...
                    <button className="px-5 py-1.5 text-sm bg-primary-orange rounded-full
                    text-white" disabled={submitting} type='submit'>{submitting ? (type === 'Create' ? 'Creating' : 'Editing') : type}</button>
                </div>
            </form>
        </section>
    );
